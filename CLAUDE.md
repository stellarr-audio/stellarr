...
- **Before tagging a release**, bump the version in three places in a single `chore: bump to vX.Y.Z` commit:
  1. `ui/package.json` — `"version"` field
  2. `ui/src/components/settings/Settings.tsx` — hardcoded `infoVersion` string
  3. `ui/package-lock.json` — regenerated by `cd ui && npm install`
- Merge the bump PR, then tag from main (`git tag vX.Y.Z`) and `gh release create vX.Y.Z` with changelog body. Changelog lives in the GitHub release body only — no `CHANGELOG.md` in-repo.
- Future automation (open): wire Settings `infoVersion` from a Vite-injected `__APP_VERSION__` so only `ui/package.json` needs bumping.
