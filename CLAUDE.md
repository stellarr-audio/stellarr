...
2. **Local code review before commit.** Run `codex review --uncommitted` for non-trivial changes and address findings inline. (Codex picks its current default model; pin one explicitly with `-c model=<name>` only if there's a reason.) Catches contrast / design-system / specificity issues, drift between docs and code, and missed edge cases before the cloud Codex bot sees them, saving review round-trips. Run for: engine logic, UI behaviour or styling, multi-file refactors, **and docs that describe code behaviour, command names, file paths, or configuration** (e.g. user manual pages, testing checklists). Skip for: typo / grammar / wording-only edits, dependency bumps, version chores, trivial style tweaks.
